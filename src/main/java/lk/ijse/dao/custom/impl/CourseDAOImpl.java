...
//    }
    ...
    public String generateProgramId() {
        ...
        String prefix = "C"; // default prefix
        int newIdNum = 1;

        if (lastId != null) {
            // prefix detect කරන්න (letters only)
            prefix = lastId.replaceAll("\\d", ""); // letters only
            String numPart = lastId.replaceAll("\\D", ""); // numbers only
            if(!numPart.isEmpty()){
                newIdNum = Integer.parseInt(numPart) + 1;
            }
        }

        // formatted id return කරන්න
        return String.format("%s%03d", prefix, newIdNum);
    }
